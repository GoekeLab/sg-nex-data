...
#7–12+ million full-length mapped readsper flow cell on MinION/GridION; 50+ million mapped reads per flow cell on PromethION
...
(c(cost.PCRcDNA.single,cost.PCRcDNA*(2:12))+cost.promethion+c(0,rep(cost.multiplex.PCRcDNA,11)))/(1:12), (cost.directRNA*(1:12)+cost.promethion_12*(1:12))/(1:12),
...
p_expected_data_cost <- ggplot(data_cost, aes(x = samples, y = cost))+
    ...
    scale_shape_manual(values = c(16,17,15), 
                       breaks = c("PromethION","GridION","Illumina"), 
                       name = "Platform")+
...
p_error_rate_log10 <- ggplot(plotdata[special_prep != 4], aes(x = protocol_type_factor, y = error_rate))+
    geom_flat_violin( aes(fill = protocol_type_factor, col = protocol_type_factor),
        alpha = 0.5,position=position_nudge(0.25), adjust = 2)+
    geom_boxplot( aes(fill = protocol_type_factor), 
                 width = 0.05, outlier.alpha = 0,position=position_nudge(0.25))+
   geom_point(aes(col = protocol_type_factor,shape = as.factor(special_prep)),
                position = position_jitter(0.2),  
...
cvr_agg <- unique(cvr_dt[grepl("allSpik",runname),list(cumEstAve=median(cn),
                                                      cumEstSD = sd(cn)), by = list(protocol_type,cellLine, txCvr_round)])  
...
p_transcript_diversity_boxplot_by_geneLength <- ggplot(dt_agg_geneLength[geneRank_new == 31063], aes(x = reorder(protocol_reduced,cumEstFun),  y = cumEstFun*100))+
    geom_boxplot()+
    stat_compare_means(comparisons = my_comparisons,
                       label.y = seq(0.4,0.2,length.out = 7)*100)+ 
     labs(x = "Protocol", y = "Percent of transcription \n for genes <= 1kb")+
    theme_classic()
...
saveRDS(data_reads, file = "first_last_exon_coverage_filtered_version.rds")
```


```{r}
data_reads <- readRDS("first_last_exon_coverage.rds")
...
## spliced reads only
data_reads <- readRDS("first_last_exon_coverage_splicedOnly.rds")
data <- do.call("rbind",data_reads)
data[, protocol := gsub("Stranded","", protocol)]
...
# 1/50bp
data_reads <- readRDS("first_last_exon_coverage_splicedOnly_1bp_50bp.rds")
data <- do.call("rbind",data_reads)
data[, protocol := gsub("Stranded","", protocol)]
...
data_reads <- readRDS("first_last_exon_coverage_filtered_version.rds")
data <- do.call("rbind",data_reads)
data[, protocol := gsub("Stranded","", protocol)]
filter_threshold <- 30
...
p_first_last_exon_coverage <- ggplot(dataLong, aes(x = protocol, y = value/nTotal, fill = factor(variable, levels = c("firstExon","lastExon","firstlastExon"))))+
    ...
    theme_classic()+
    theme(legend.position = "top")
pdf("fraction_first_last_exon_coverage.pdf", width = 6, height = 4)
