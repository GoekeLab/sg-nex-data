...
novelTxCount_combined <- do.call("rbind", list(novelTxCount[ndr_value %in% seq(0.1,1, by = 0.1) ], novelTxCountUnique[(round(ndr_value,1) == 0.6)|(ndr_value %in% seq(0.1,1, by = 0.1))]))
...
np <- lapply(seList[19], function(ss) {
    ...
    writeToGTF(temp_anno, file = paste0(".",gsub(".rds",".gtf",basename(ss))))
})
```
...
```{r}
library(data.table)
sqanti3_corrected_gtf <- dir(".", pattern = "gtf_strandCorrected", full.names = TRUE)
write.table(sqanti3_corrected_gtf, file = "sqanti3_annotation.txt", sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
```
...
```{bash}
cat sqanti3_annotation.txt | | while read line
do  
