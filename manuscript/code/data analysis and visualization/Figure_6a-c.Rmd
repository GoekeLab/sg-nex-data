...
splicingDT <- do.call("rbind",lapply(cellLineVec, function(s){
  dt <- data.table(as.data.frame(rowData(seOutput)))#data.table(as.data.frame(rowData(seOutput[which(apply(assays(seOutput)$CPM>=1,1,sum)>=1)])))#
setnames(dt, c("TXNAME","GENEID"),c("tx_name","gene_name"))
dt[, txCount:=length(unique(tx_name)), by = gene_name] # txCount should be based on active isoforms, active across samples, at least 1 cpm in any samples
...
# dt <- lr_ave[cellLine==s,.(tx_name,gene_name, aveExp,cpm.major)][dt,on = c("tx_name","gene_name")] # for salmon and nanocount, there is no fulllengthcounts
# setnames(dt, c("aveExp","cpm.major"),
#          paste0("LongRead.",c("cpm","cpm.major")))
...
p_within_cellLine <- ggplot(plotdata, aes(x = variable, y = count))+
  ...
  scale_x_discrete(limits = rev(c("exonSkipping",
                              ...
                              "altSplicing5Prime")))+
  theme_classic()
# pdf(paste0("/mnt/projects/SGNExManuscript/revisionFigures_guppy6.4.2/figure4/splicing_summary_withinCellLines.pdf"), width = 13, height = 4)
# print(p)
# dev.off()
# total percentage 
...
```{r}
#splicingDT <- readRDS("splicing_summaryDT_withinCellLine_bambu_filterBycpm.rds")
...
# barplot_top[is.na(barplot_top)] <- 0
# ht <- UpSet(m, top_annotation = upset_top_annotation(m,
...
sbox <- as.matrix(dcast(plotdata, variable ~ cellLine, value.var = "count")[match(colnames(lt),variable),2:6,with=FALSE]) #match(set_name(m)[order(ss, decreasing = TRUE)], variable)
...
ht = UpSet(m,
    ...
    top_annotation = HeatmapAnnotation(
        ...
        # "10>=" = anno_barplot(cs10,
        #                        ylim =c(0, max(cs10)*1.1),#
        #                        border = FALSE,
        #                        gp = gpar(fill = brewer.pal(8,"Dark2")[c(1:2)]),
        #                        height = unit(4, "cm")
        # ),
...
# decorate_annotation("Event Intersections", {
#     grid.text(cs[od], x = seq_along(cs), y = unit(cs[od], "native") + unit(2, "pt"),
#         default.units = "native", just = c("left", "bottom"),
#         gp = gpar(fontsize = 6, col = "#404040"), rot = 45)
# })
# pdf(paste0("/mnt/projects/SGNExManuscript/revisionFigures_guppy6.4.2/figure4/DE_IsoSwtType_UpSetPlot_withinCellLines_novel",saveDate,"_bambu_filterByCPM.pdf"), width = 12, height = 10)
ht = draw(ht)
od = column_order(ht)
# decorate_annotation("Event Intersections", {
#     grid.text(cs[od,1], x = seq_along(cs), y = unit(cs[od], "native") + unit(2, "pt"),
#         default.units = "native", just = c("left", "bottom"),
#         gp = gpar(fontsize = 6, col = "#404040"), rot = 45)
# })
# dev.off()
...
```{r}
#dtuGenes_1vs1 <- readRDS(paste0("/mnt/projects/SGNExManuscript/output_guppy6.4.2/dtuGenes_1vs1.rds"))
dtuGenes_1vsall <- readRDS(paste0("/mnt/projects/SGNExManuscript/output_guppy6.4.2/dtuGenes_1vsall.rds"))
table_1vsall <- fread(paste0("/mnt/projects/SGNExManuscript/output_guppy6.4.2/spliced_output_1vsall_bambu.csv"), header = TRUE)
tss_threshold <- 0
tes_threshold <- 0
...
# geneTxTable <- txLengths.tbldf[,.(tx_name, gene_id, hgnc_symbol,gene_biotype)]
# setnames(geneTxTable, "gene_id","gene_name")
...
# write.csv(rowInfo, file = paste0("/mnt/projects/SGNExManuscript/output_guppy6.4.2/splicing_heatmap_rowinfo.csv"))
...
hgncTypes = columnAnnotation(newTxClass = as.factor(rowInfo$newTxClassAggregated),
                           ...
                           # altTSS = as.factor(rowInfo$TSS),
                           # altTES = as.factor(rowInfo$TES),
  ...
  #show_legend = c(rep(TRUE,11),FALSE))
...
print(p)
pdf(paste0("/mnt/projects/SGNExManuscript/revisionFigures_guppy6.4.2/figure4/DmntIsoSwt_Heatmap17Aug2023.pdf"), width = 13, height = 12)#
p
dev.off()
```
...
## arrange main figure
```{r}
pdf(paste0("/mnt/projects/SGNExManuscript/revisionFigures_guppy6.4.2/figure4/Figure5_draft_17Aug2023.pdf"), width = 8.2, height = 20)#
...
p_within_cellLine_nanocount <- ggplot(plotdata_nanocount, aes(x = variable, y = count))+
  ...
  scale_x_discrete(limits = rev(c("exonSkipping",
                              ...
                              "altSplicing5Prime")))+
  theme_classic()
# pdf(paste0("splicing_summary_withinCellLines.pdf"), width = 13, height = 4)
# print(p)
# dev.off()
# total percentage 
...
```{r}
library(dplyr)
#saveRDS(splicingDT, file = paste0("output/splicingDT.rds"))
splicingDT <- readRDS("splicing_summaryDT_withinCellLine.rds")
...
plotdata_nisoforms <- do.call("rbind",lapply(genevec, function(s){
  ...
  # eventSummary[, others := paste0(splicingDT[gene_name == s & value]]
  return(eventSummary)
}))
...
p_gene_rank_nisoforms <- ggplot(tmpdata, aes(x = gene_name))+
  ...
  # geom_point(data = plotdata_long[variable == "altFirstExon"], aes(x = gene_name, y = value), stat = "identity", col = colPat[1], alpha = 0.5)+
...
# pdf(paste0("figures/DE_IsoSwtType_GeneRanks",saveDate,".pdf"), width = 12, height = 5)
# print(p)
# dev.off()
...
#c('Hct116','HepG2','K562','A549','MCF7',"H9","HEYA8")
...
# exonsRanges$exon_rank_adjust <- tmp$exon_rank_adjust
...
#design_full <- model.matrix(~cellLine, data=DRIMSeq::samples(dFilter))
...
# geneTxTable <- as.data.table(rowData(seOutput))
# setnames(geneTxTable, c("GENEID","TXNAME"),c("gene_name","tx_name"))
# geneTxTable[, nisoform:=length(unique(tx_name)), by = gene_name]
geneTxTable <- geneTxTableRaw[match(rownames(count.data),tx_name)]
na.id <- which(is.na(geneTxTable$tx_name))
if(length(na.id)>0){
  count.data <- count.data[-na.id,]
...
#setnames(geneTxTable_extended, "gene","gene_name")
...
```{r}
library(edgeR)
# choose two cell lines: start from salmon
# filesList <- list.files("/mnt/processMachine2/chenying/salmon_fastq6.4.2/count/","quant.sf",recursive = TRUE,full.names = TRUE)
# filesList <- list.files("/mnt/processMachine2/chenying/salmon_fastq6.4.2/count/","quant.sf",recursive = TRUE,full.names = TRUE)
...
#y1 <- calcNormFactors(y) # does not have any effect now post and pre same value 
...
# counts$gene_id <- tmp_wide$gene_name
...
# gene_cts <- trs_cts_unfiltered %>% 
#     dplyr::select(c(1, 3:ncol(trs_cts)))  %>% 
#     group_by(gene_id) %>% 
...
dFilterList1vsall <- lapply(cellLineVec, function(s){
  print(s)
    sample.data <- unique(com_data_filter_fullLength[,.(runname, protocol,cellLine)])#nanocountLR
#sample.data <- unique(salmonLR[,.(runname, protocol,cellLine)])#nanocountLR
sample.data <- sample.data[match(colnames(count.data)[-c(1,2)],runname)]
sample.data[, ref:=as.numeric(cellLine==s)]
sample.data[, ref:=factor(ref)]
sample.data <- setDF(sample.data)
...
#plotMDS(yobj,col = c(1:2)[yobj$samples$group],labels = basename(colnames(yobj$samples)),xlim = c(-4,4))
...
```{r}
# dtuGenes_1vs1 <- readRDS(paste0("dtuGenes_1vs1_salmonLR.rds"))
# dtuGenes_1vsall <- readRDS(paste0("dtuGenes_1vsall_salmonLR.rds"))
...
outDt_1vs1 <- do.call("rbind",lapply(seq_len(ncol(combMat)), function(ss){
    ...
    #dexseq_temp2[, log2fold_1_0 := -(log2fold_1_0)]
...
# outDt_wide_dmn <- dmnTx[outDt_wide, on = c("tx_name","cellLine")]
# outDt_wide_dmn_fl <- fLTx[outDt_wide_dmn, on = c("gene_name","tx_name","cellLine")]
# outDt_wide_dmn_fl
...
# it's best if we could find out for the ones identified by each, what is the supporting status 
