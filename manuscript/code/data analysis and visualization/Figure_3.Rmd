...
# com_data <- do.call("rbind", c(list(com_data), lapply(countList, "[[",1)))
# com_data_gene <-do.call("rbind", c(list(com_data_gene), lapply(countList, "[[",2))) 
```
...
# mat_cor <- mat_cor[(within_cellLine &(match_status == TRUE))|(!within_cellLine)]
   ...
   ave_data[, agg_gene_cluster := ifelse(gene_cluster == "processed_transcript", "lncRNA",
    ...
    p_protein_coding <- ggplot(plotdata1[agg_gene_cluster == "protein_coding"], aes(x = directcDNA, y = Illumina))+
     ...
     p_lncRNA <- ggplot(plotdata1[agg_gene_cluster == "lncRNA"], aes(x = directcDNA, y = Illumina))+
        ...
        # scale_fill_gradient2(limits = c(0, 50), oob = scales::squish)+
         #geom_density_2d()+
        # geom_hex(binwidth = c(0.05, 0.05), col = "steelblue",size = 0.5, alpha = 0.1)+
         ggpubr::stat_cor(aes(label = ..r.label..),method = "spearman", cor.coef.name = "Sp.R")+ 
         xlim(c(0,14))+
         ylim(c(0,14))+
...
# comDataTranscriptFiltered <- comDataTranscript[method %in% c("bambu_lr","salmon_lr","rsem_sr","salmon_sr")&(!grepl("PacBio",protocol_general))]
lr <- "salmon_lr"
sr <- "salmon_sr"
gene <- TRUE
data_type <- "all"
...
# 
# hm_grob <-grid.grabExpr(draw(p_heatmap[[1]])) 
...
p_gene_spikein_abe_suppl <- ggplot(si_data_gene, aes(x=ae, fill = method)) + #"PacBio",
...
# pList_gene_combined <- do.call("c",list(pList_gene, list(protocol_annotation, method_annotation)))
library(gridExtra)
...
p_gene_cellLine_scatter_suppl1 <- ggplot(plotdata2[agg_gene_cluster=="protein_coding"], aes(x = value, y = Illumina))+
        # geom_abline(intercept = 0, slope = 1)+
        # geom_hex(binwidth = c(0.05, 0.05), col = "steelblue", alpha = 0.1)+
    geom_abline(intercept = 0, slope = 1)+
        geom_hex(binwidth = c(0.1, 0.1))+ #col = "steelblue", size = 0.5, alpha = 0.1
        scale_fill_steps(breaks = c(10,100,1000),low = "steelblue1", high = "steelblue4",trans = "log10")+
        # scale_fill_gradient2(limits = c(0, 50), oob = scales::squish)+
...
matCorData <- do.call("rbind",lapply(seq_len(nrow(geneCellLineMetrics)), function(k){
    ...
    mat_cor <- readRDS(paste0("/mnt/projects/SGNExManuscript/output_guppy6.4.2/replicateResults/replicate_gene_expression_comparison_",lr,"_",sr,"_",metric,saved_date,".rds"))
...
saveRDS(matCorData, file = "/mnt/projects/SGNExManuscript/output_guppy6.4.2/replicateResults/matCorData_7Aug2023.rds")

matCorData <- readRDS("/mnt/projects/SGNExManuscript/output_guppy6.4.2/replicateResults/matCorData_7Aug2023.rds")
...
<!-- ## f gene heatmap with dendrograms using other methods  -->
<!-- ```{r} -->
...
<!-- pList <- plot_heatmap(dt = comDataGeneFiltered, methodNames = c(lr, sr), gene, data_type,genevec,ensemblAnnotations.transcripts , samples, number_of_genes, runnamevec) -->
<!-- ``` -->
...
pdf("/mnt/projects/SGNExManuscript/revisionFigures_guppy6.4.2/figure3/suppl_figure2_draft_4Sep2023.pdf", width = 14, height = 20)
ggdraw() +
...
temp[grepl("GIS", rn ),rn := samples[match(rn, old_runname)]$runname]
dt_wide <- dcast(temp, gene_name ~ paste0(rn,method), value.var = "normEst")
# dt_wide <- dcast(comDataGene[method %in% c("salmon_lr","salmon_sr") &(gene_name %in% unique(ensemblAnnotations.transcripts[gene_biotype == "protein_coding"]$gene_name))&(runname %in% runnamevec)], gene_name ~ runname, value.var = "normEst")
    corMatrix <- cor(log2(dt_wide[,-1,with = FALSE]), method = "spearman", use = "pairwise.complete.obs")
...
```{r}
stats.pychopper <- fread("/mnt/projects/SGNExManuscript/output_guppy6.4.2/adapter_analysis/pychopper_results/cdna/statistics.tsv", header = TRUE)
...
ggdraw() +
    ...
    #  draw_plot(protocol_annotation_tx,1/4,0,1/8,1/5)+
    # draw_plot(method_annotation_tx,3/8,0,1/8,1/5)+
...
```{r}
pdf("/mnt/projects/SGNExManuscript/revisionFigures_guppy6.4.2/figure3/r3.15.pdf", width = 14, height = 14)
...
noprint <- lapply(seq_len(nrow(methodNamesList)), function(k){
    ...
    number_of_genes <- 10000
    if(k%%3==1){
        pdf(paste0(wkdir,"revisionFigures_guppy6.4.2/geneHeatmaps/",lr,"_",sr,"_", c("transcript","gene")[gene+1],"_",number_of_genes,"_25May.pdf"), width = 8, height = 5)
    }
    if(gene){
