...
local_path <- "RunBambu17Apr_junction/"
...
juncData <- lapply(seq_along(sampleNames), function(r){
    system(paste0("aws s3 cp ", bam.file[r]) " ",local_path))
...
saveRDS(data_lr, file = paste0(" juncLR_May5_updatedsample109.rds"))
...
juncData <- lapply(sr_runNames, function(r){
    ...
    if(grepl("H9|HEYA8",r)){
        ...
        bamFile = sampleData_runName$star_map_genomeBam.path
    }
    if(length(bamFile)==0){
        return(NULL)
    }
